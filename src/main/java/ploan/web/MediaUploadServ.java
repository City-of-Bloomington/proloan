...
import ploan.utils.*;
//
@WebServlet(urlPatterns = {"/MediaUpload","/MediaUploadServ"})
...
public class MediaUploadServ extends TopServlet{
	...
	try{
		...
		while (iter.hasNext()) {
			...
			if(ext.equals("")){
			    ...
			    if(!ext.equals("")){
				file_name = media.updateFileName(ext);
				File newFile = new File(file.getParent(), file_name);
	...
	//
	out.println("<html><head><title>Proloan Images</title>");
	...
	out.println(" <center><h3> Attachments</h3>");
	...
	//
	//
	out.println("<tr><td align=\"center\"><b>Upload New Attachment</b></td></tr>");
	//
	// 1st block
	//
	out.println("<tr><td>");
	...
	out.println("<tr><td colspan=\"2\"><font color=\"green\">To upload a new file, please consider the following:<uL>"+
		    " <li>You can upload media, document, spreadsheet files</li>"+
